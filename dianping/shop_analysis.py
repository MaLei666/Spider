...
def data_analysis():
    ...
    for each in class_all:
        shop_sql='SELECT shopName,mainRegionName,taste,service,avgPrice FROM food_rank WHERE class="{}";'.format(each[0])
        # print(shop_sql)
...
def get_shop_list():
    ...
    for shop_list in shop_list_all[:1]:
        for shop in shop_list:
            ...
            services.append(shop[3])
            avgPrices.append(shop[4])
        ...
        data = pandas.concat([shopNames, tastes,services,avgPrices], axis=1, ignore_index=True)
        data.columns = ['shopNames', 'tastes','services','avgPrices']
        print(data)

# get_shop_list()

def plot_graph(shopNames, tastes,services,avgPrices):
    # xnew = np.linspace(shopNames,300)  # 300 represents number of points to make between T.min and T.max
    power_smooth = spline(shopNames,power,xnew)
